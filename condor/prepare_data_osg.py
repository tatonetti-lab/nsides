...
for reportblock in range(0,thisReportBlock_array.shape[0],blockSize):
    start_point = reportblock
    end_point = reportblock + blockSize

    if (end_point > thisReportBlock_array.shape[0]):
        end_point = thisReportBlock_array.shape[0]


    io.mmwrite("model_"+str(runIndices[args.model_num])+"_"+str(fileNum)+"_reports.mtx",thisReportBlock_array[start_point:end_point,],field='integer')
