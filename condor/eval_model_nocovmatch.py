...
runIndices = [
    ...
    2803,2811,2788,2835,2717,2238,2236,3180,3669,4215,3000,2452
]
...
if args.model_type == 'nopsm':
    ...
    output = open('results_nreports_'+str(args.model_type)+save_string+'.pkl','wb')
    ...
    output = open('results_'+str(args.model_type)+save_string+'.pkl','wb')
...
comb_scores = comb_scores[0:1535879]
...
def calcbin(outcome=1, binLo=0.0, binHi=0.10):
    x_indices_posoutcome_outcbin = np.where( (resu_exp == outcome) & (outc_exp <= binHi) & (outc_exp > binLo))[0]
...
while 1:
    ...
    if thisBinPos < 10 or ratio < 10.0:
        ...
        print lobin, hibin, len(posbins), len(negbins)
